...
void printHelp()
{
    fmt::println(R"(Usage: sam_hq <encoder_path> <decoder_path> <image> [<output>]

    Model Code: https://github.com/SysCV/sam-hq
...
private:
    ...
    const std::string m_window = "High Quality SAM Demo";
    ...
    struct EncodedImageTensor {
        ...
        Ort::Value image_tensor{nullptr};
        Ort::Value interm_tensor{nullptr};
    };
...
void Demo::encodeImage()
{
    ...
    io_binding.BindOutput("image_embeddings", m_encoded_image.image_tensor);
    io_binding.BindOutput("interm_embeddings", m_encoded_image.interm_tensor);

    fmt::print("Running HQ SAM image encoder... ");
...
void Demo::runInference()
{
    ...
    fmt::print("Running HQ SAM decoder with prompts... ");
    ...
    io_binding.BindInput("interm_embeddings", m_encoded_image.interm_tensor);
